...
static EZOS_TASK(task_fatfs_test);
...
EZOS_TASK(task_fatfs_test)
{
    ...
    if (buf != NULL)
    {
        ...
        // 创建文件, 写测试
        ff_flag = f_open(&fdst, "0:/hello/测试.bin", FA_CREATE_ALWAYS | FA_WRITE);
        uart_printf(COM1, "f_open: %u\r\n", ff_flag);

        for (i=0; i<BUF_SIZE; i++) buf[i] = i;
        time = delay_tick_get();
        for (i=0; i<FLOOPS; i++) f_write(&fdst, buf ,BUF_SIZE, &num);           // 写 测试.bin 文件
        ...
        // 读测试
        ff_flag = f_open(&fsrc, "0:/hello/测试.bin", FA_OPEN_EXISTING | FA_READ);
        uart_printf(COM1, "f_open: %u\r\n", ff_flag);

        time = delay_tick_get();
        for (i=0; i<FLOOPS; i++) f_read(&fsrc, buf ,BUF_SIZE, &num);            // 读 测试.bin 文件
        time = delay_tick_get() - time;
