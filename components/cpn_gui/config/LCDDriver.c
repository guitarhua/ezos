// 用户自定LCD接口驱动, 若使用emWIN内置LCD驱动, 则无需此文件
...
// GUIDriver 用户优化, 提高刷屏速度. 建议仅当视域屏和实际屏保持一致时使用!
...
/*
**********************************************************************
*   写点函数, 最重要的函数. 实际项目请自行优化提高显示速度!
**********************************************************************
*/
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) {
    // 避免某些编译器出现 pDevice 未使用的警告
...
**********************************************************************
*   读点函数, 重要函数之一.
*   若实际项目不需要用到 反色/透明/半透明 效果时, 可不用此函数
**********************************************************************
*/
...
static void  _DrawBitLine8BPP(GUI_DEVICE * pDevice, int x, int y, U8 const GUI_UNI_PTR * p, int xsize, const LCD_PIXELINDEX * pTrans) {
  ...
  switch (GUI_pContext->DrawMode & (LCD_DRAWMODE_TRANS | LCD_DRAWMODE_XOR)) {
  case 0:
    // 此处可优化, 使用 lcd_chip_canvas, 及 lcd_chip_wring...
    if (pTrans) {
