...
#ifdef STM32_ETH_UIP_ENABLE
/**
  ******************************************************************************
  * -V1.0.0, 2013.1.11, mousie
  *          - 属于STM32F105/STM32F107 网络底层硬件驱动配置.
  *          - 支持 MII / RMII 数据接口, 二选一.
  *          - 可选择是否使用 MCO 口. 即通过STM32芯片引脚给提供CLK信号.
  *          - 此驱动用于支持 uIP 组件. 请勿直接调用.
  *          - 硬件上, 需要通过 MII 或 RMII 接口, 外接物理层芯片 DP83848.
  ******************************************************************************
  */
#define STM32_ETH_PRINTF(...)           assert_printf(__VA_ARGS__)

#define PHY_ADDRESS                     0x01                                    // 物理层芯片(DP83848)地址
#define ETH_RXBUFNB                     2                                       // 接收缓冲区帧数
#define ETH_TXBUFNB                     2                                       // 发送缓冲区帧数
...
void GPIO_Configuration(void)
{
    ...
    // 使用MCO引脚
    #ifdef ETH_MCO_ENABLE
        /* Get HSE clock = 25MHz on PA8 pin(MCO) */
        RCC_MCOConfig(RCC_MCO_HSE);
    #endif
#elif defined ETH_RMII_MODE
    // RMII模式
    ...
    - ETH_MII_TXD2:                         PC2
    - ETH_MII_TX_CLK:                       PC3
    - ETH_MII_RX_DV     / ETH_RMII_CRS_DV:  PA7 (or remap to PD8)
    - ETH_MII_RXD0      / ETH_RMII_RXD0:    PC4 (or remap to PD9)
    - ETH_MII_RXD1      / ETH_RMII_RXD1:    PC5 (or remap to PD10)
    - ETH_MII_RXD2:                         PB0 (or remap to PD11)
    - ETH_MII_RXD3:                         PB1 (or remap to PD12) */
...
#ifdef ETH_PTP_ENABLE
    // ETH_MII_PPS_OUT / ETH_RMII_PPS_OUT
    GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_5;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_AF_PP;
    GPIO_Init(GPIOB, &GPIO_InitStructure);
#endif // ETH_PTP_ENABLE
...
// stm32 网络接口初始化函数
void stm32_eth_init(uip_macaddr_t mac)
{
    ...
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = (ETH_IRQ_PRIORITY & 0x0C) >> 2;
    NVIC_InitStructure.NVIC_IRQChannelSubPriority = (ETH_IRQ_PRIORITY & 0x03);
    ...
    // 非自适应时, 手动配置工作模式, 全双工模式, 否则为半双工模式
    //ETH_InitStructure.ETH_Mode = ETH_Mode_FullDuplex;

    // 非自适应时, 手动配置网络速率, 100M, 否则为10M网速
    ...
    ETH_MACAddressConfig(ETH_MAC_Address0, (u8*)&mac.addr[0]);

    /* Enable MAC and DMA transmission and reception */
    ETH_Start();
}

// stm32检查网络连接状态. 0-未连接, 1-已连接.
uint8_t stm32_eth_linked(void)
{
    uint16_t reg = ETH_ReadPHYRegister(PHY_ADDRESS, PHY_BSR);

    // 1, on. 0, off
    if (reg & PHY_Linked_Status) return 1;
    else                         return 0;
}
...
{
    // 接收到数据
    if ((ETH->DMASR & ETH_DMA_FLAG_R) != (u32)RESET )
    {
        uip_chip_irq_rx();
    }
...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  {
    STM32_ETH_PRINTF("\r\n-- stm32_eth auto negotiation start.\r\n");
    ...
    if((RegValue & PHY_Duplex_Status) != (uint32_t)RESET)
    {
      ...
      STM32_ETH_PRINTF("   stm32_eth full duplex.\r\n");
    }
    else
    {
      /* Set Ethernet duplex mode to HalfDuplex following the autonegotiation */
      ETH_InitStruct->ETH_Mode = ETH_Mode_HalfDuplex;
      STM32_ETH_PRINTF("   stm32_eth half duplex.\r\n");
    }
    ...
    if(RegValue & PHY_Speed_Status)
    {
      ...
      STM32_ETH_PRINTF("   stm32_eth 10M speed.\r\n");
    }
    else
    {
      /* Set Ethernet speed to 100M following the autonegotiation */
      ETH_InitStruct->ETH_Speed = ETH_Speed_100M;
      STM32_ETH_PRINTF("   stm32_eth 100M speed.\r\n");
    }
    STM32_ETH_PRINTF("   stm32_eth auto negotiation ok.\r\n\r\n");
  }
  else
  {
    ...
    {
      STM32_ETH_PRINTF("   stm32_eth init error.\r\n\r\n");
      /* Return ERROR in case of write timeout */
      return ETH_ERROR;
    }
