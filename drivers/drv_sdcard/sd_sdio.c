/**
  ...
  *             state by using the SD_WaitOperation() function. This function checks
...
#if (defined SDCARD_MODE_SDIO) || (defined SDFLASH_MODE_SDIO)
/**
  ******************************************************************************
  * -V2.0.0, 2012.7.5, mousie
  *          - 属于SD卡SDIO底层硬件驱动配置. 使用SDIO 4bit传输模式.
  *          - 取消了原驱动的部分函数. 仅支持 SD_DMA_MODE
  *          - 对原驱动的块读写函数进行了大幅度优化, 加快读写速度.
  *          - 此驱动用于支持 drv_sdcard 及 drv_sdflash. 请勿直接调用.
  *          - 支持 SD_V1.x SD_V2 SDHC 卡, 最大可支持4G卡.
  *          - 仅支持512的块大小. 1024以上的块大小初始化, 读写错误.
  *          - 此驱动用于支持 drv_sdcard 及 drv_sdflash. 请勿直接调用.
  ******************************************************************************
  */
// 初始化时钟必须小于400KHz, 计算公式如下:
// SDIO_CK = SDIOCLK / (SDIO_INIT_CLK_DIV + 2)
// SDIOCLK 为 HCLK 时钟, 一般为72MHz
#define SDIO_INIT_CLK_DIV               0xB2

// 传输模式时钟必须小于25MHz, 计算公式如下:
// SDIO_CK = SDIOCLK / (SDIO_TRANSFER_CLK_DIV + 2)
// SDIOCLK 为 HCLK 时钟, 一般为72MHz
#define SDIO_TRANSFER_CLK_DIV           0x01
...
/**
  * @brief  Initializes the SD Card port and NVIC
  * @param  None
  * @retval None
  */
void SD_Port_Init(void)
{
  ...
  NVIC_InitStructure.NVIC_IRQChannel = SDIO_IRQn;
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = (SDIO_IRQ_PRIORITY & 0x0C) >> 2;
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = (SDIO_IRQ_PRIORITY & 0x03);
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
  NVIC_Init(&NVIC_InitStructure);
}
...
/** @defgroup STM32_EVAL_SDIO_SD_Private_Variables
  * @{
  */
static uint32_t CardType = SDIO_UNKNOW;
static uint16_t SD_BlockSize = 512;
static uint8_t  SD_Power = 9;
...
/**
  ...
  */
SD_Error SD_Init(uint16_t BlockSize)
{
  ...
  if (((BlockSize % 512) != 0) || (BlockSize > 2048))
    return SD_ERROR;

  SD_BlockSize = BlockSize;
  SD_Power = 8 + (BlockSize / 512);
  ...
  if (errorstatus == SD_OK)
  {
    ...
    SDIO_CmdInitStructure.SDIO_Argument = (uint32_t)SD_BlockSize;               // 设置SD卡块大小
...
/**
  * @brief  Gets the cuurent sd card status.
  * @param  None
  * @retval SDCardState: SDCard state.
  */
SDCardState SD_GetState(void)
{
  uint32_t resp1 = 0;

  if (CardType == SDIO_UNKNOW) return SD_CARD_ERROR;
...
/**
  ...
  * @param  ReadAddr: Block Address from where data are to be read.
  * @retval SD_Error: SD Card Error code.
  */
SD_Error SD_ReadBlock(uint8_t *readbuff, uint32_t ReadAddr)
{
  ...
  if ((CardType == SDIO_UNKNOW) || (readbuff == 0)) return SD_ERROR;

  TransferError = SD_OK;
  TransferEnd = 0;
  StopCondition = 0;
  SDIO->DCTRL = 0x0;
  if (CardType != SDIO_HIGH_CAPACITY_SD_CARD) ReadAddr <<= SD_Power;

  SDIO_DataInitStructure.SDIO_DataTimeOut = SD_DATATIMEOUT;
  SDIO_DataInitStructure.SDIO_DataLength = SD_BlockSize;
  SDIO_DataInitStructure.SDIO_DataBlockSize = (uint32_t) SD_Power << 4;
  ...
  SDIO_DMACmd(ENABLE);
  SD_LowLevel_DMA_RxConfig((uint32_t *)readbuff, SD_BlockSize);

  while ((SD_DMAEndOfTransferStatus() == RESET) && (TransferEnd == 0) && (TransferError == SD_OK));
  errorstatus = SD_WaitOperation();
  if (errorstatus != SD_OK) TransferError = SD_ERROR;
...
/**
  ...
  * @retval SD_Error: SD Card Error code.
  */
SD_Error SD_ReadMultiBlocks(uint8_t *readbuff, uint32_t ReadAddr, uint32_t NumberOfBlocks)
{
  SDIO_CmdInitTypeDef SDIO_CmdInitStructure;
  SDIO_DataInitTypeDef SDIO_DataInitStructure;
  SD_Error errorstatus;

  if ((CardType == SDIO_UNKNOW) || (readbuff == 0) || (NumberOfBlocks <= 1)) return SD_ERROR;
  ...
  SDIO_DataInitStructure.SDIO_DataTimeOut = SD_DATATIMEOUT;
  SDIO_DataInitStructure.SDIO_DataLength = NumberOfBlocks << SD_Power;
  SDIO_DataInitStructure.SDIO_DataBlockSize = (uint32_t)SD_Power << 4;
  SDIO_DataInitStructure.SDIO_TransferDir = SDIO_TransferDir_ToSDIO;
  SDIO_DataInitStructure.SDIO_TransferMode = SDIO_TransferMode_Block;
  SDIO_DataInitStructure.SDIO_DPSM = SDIO_DPSM_Enable;
  ...
  SDIO_ITConfig(SDIO_IT_DATAEND, ENABLE);
  SDIO_DMACmd(ENABLE);
  SD_LowLevel_DMA_RxConfig((uint32_t *)readbuff, (NumberOfBlocks << SD_Power));
...
/**
  ...
  * @param  WriteAddr: Block Address from where data are to be read.
  * @retval SD_Error: SD Card Error code.
  */
SD_Error SD_WriteBlock(const uint8_t *writebuff, uint32_t WriteAddr)
{
  SDIO_CmdInitTypeDef SDIO_CmdInitStructure;
  SDIO_DataInitTypeDef SDIO_DataInitStructure;
  SD_Error errorstatus;

  if ((CardType == SDIO_UNKNOW) || (writebuff == 0)) return SD_ERROR;

  TransferError = SD_OK;
  TransferEnd = 0;
  StopCondition = 0;
  SDIO->DCTRL = 0x0;
  if (CardType != SDIO_HIGH_CAPACITY_SD_CARD) WriteAddr <<= SD_Power;
  ...
  SDIO_ITConfig(SDIO_IT_DATAEND, ENABLE);
  SD_LowLevel_DMA_TxConfig((uint32_t *)writebuff, SD_BlockSize);
  SDIO_DMACmd(ENABLE);
...
/**
  ...
  * @retval SD_Error: SD Card Error code.
  */
SD_Error SD_WriteMultiBlocks(const uint8_t *writebuff, uint32_t WriteAddr, uint32_t NumberOfBlocks)
{
  SDIO_CmdInitTypeDef SDIO_CmdInitStructure;
  SDIO_DataInitTypeDef SDIO_DataInitStructure;
  SD_Error errorstatus;
  ...
  if ((CardType == SDIO_UNKNOW) || (writebuff == 0) || (NumberOfBlocks <= 1)) return SD_ERROR;
  ...
  SDIO_ITConfig(SDIO_IT_DATAEND, ENABLE);
  SDIO_DMACmd(ENABLE);
  SD_LowLevel_DMA_TxConfig((uint32_t *)writebuff, (NumberOfBlocks << SD_Power));
...
/**
  ...
  * @retval SD_Error: SD Card Error code.
  */
SD_Error SD_WaitOperation(void)
{
  SDCardState state;
...
{
  if (StopCondition == 1) TransferError = SD_StopTransfer();
